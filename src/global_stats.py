...
for j in range(0, 3):
    # student data
    if j == 0:
        (x_train, x_test, y_train, y_test, s_train, s_test) = load_student_data("AK", conf=criteria)
    elif j == 1:
        (x_train, x_test, y_train, y_test, s_train, s_test) = load_student_data("AK", alpha=[25,100],  conf=criteria)
    elif j == 2:
        (x_train, x_test, y_train, y_test, s_train, s_test) = load_student_data("AK")
    else:
        (x_train, x_test, y_train, y_test, s_train, s_test) = load_student_data("AK", alpha=[100,30],  conf=criteria)
    # get student init stats
    st_model = train_student(x_train, y_train, verbose=False)
    stats = fairness(st_model, x_test, y_test, s_test)
    print(stats)
    np.save("tmp/" + str(j) + "_st_init_stats.npy", stats)
    ...
    def train_students(nb_teachers, nb_fair_tchrs):
        ...
        tchrs_ensemble = Ensemble(nb_teachers, nb_fair_tchrs, criteria)
